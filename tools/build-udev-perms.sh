...
UDEV_FILE=${ROMFSDIR}/libexec/udev/rules.d/50-perms.rules
...
do
	...
	*,*) DEV=`echo $i | sed -e 's/,.*//' -e 's,.*/,,'`; DIR=`echo $i | sed -e 's/,.*//' -e 's,[^/]*$,,'`; echo "KERNEL==\"$DEV\", NAME=\"$DIR$DEV\", GROUP=\"$GROUP\", MODE=\"$PERM\"" >> $UDEV_FILE;;
	esac
done
