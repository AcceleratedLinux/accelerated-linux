...
static struct map_desc armada_380_io_desc[] __initdata = {
	{
		.virtual = (unsigned long) ARMADA_380_REGS_VIRT_BASE,
		.pfn     = __phys_to_pfn(ARMADA_380_REGS_PHYS_BASE),
		.length  = ARMADA_380_REGS_SIZE,
		.type    = MT_DEVICE,
	},
};

static void __init armada_380_map_io(void)
{
	iotable_init(armada_380_io_desc, ARRAY_SIZE(armada_380_io_desc));
}
