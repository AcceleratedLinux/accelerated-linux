...
static unsigned int sfe_cm_post_routing(struct sk_buff *skb, int is_v4)
{
	...
	if (ct->mark & 0x1000) return NF_ACCEPT;
...
#else
	if (unlikely(ct->status & IPS_UNTRACKED)) {
#endif
...
static int sfe_cm_event(struct notifier_block *this, unsigned long events,
			   void *ptr)
{
	struct nf_event *evt = (struct nf_event *)ptr;

	switch (evt->type) {
	case NF_EVT_TYPE_CT:
		return sfe_cm_conntrack_event(events, evt->item);
...
static void sfe_cm_sync_rule(struct sfe_connection_sync *sis)
{
	...
	//WARN_ON(ct->timeout.data != (unsigned long)ct);
	...
	case IPPROTO_UDP:
		...
		if (!test_bit(IPS_ASSURED_BIT, &ct->status) && acct) {
			...
			if (reply_pkts != 0) {
				...
				//l4proto = nf_ct_l4proto_find((sis->is_v6 ? AF_INET6 : AF_INET), IPPROTO_UDP);
				//timeouts = nf_ct_timeout_lookup(&init_net, ct, l4proto);
				//l4proto = nf_ct_l4proto_find(IPPROTO_UDP);
