...
static int ctnetlink_notification(struct notifier_block *this,
				  unsigned long events, void *ptr)
{
	...
	switch (evt->type) {
	case NF_EVT_TYPE_CT:
		return ctnetlink_conntrack_event(events, evt->item);

	case NF_EVT_TYPE_EXP:
		return ctnetlink_expect_event(events, evt->item);
...
static struct notifier_block ctnl_notifier = {
	.notifier_call = ctnetlink_notification,
};
#else
