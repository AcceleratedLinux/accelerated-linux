...
static void dump_realm(struct nf_log_buf *m,
			    ...
			    const struct sk_buff *skb)
{
	...
	if (dst) {
		nf_log_buf_add(m, "REALM=0x%x ", dst->tclassid);
	}
}
#endif
...
static void dump_conntrack(struct nf_log_buf *m,
			    const struct nf_loginfo *info,
			    const struct sk_buff *skb)
{
	...
	if (ct == NULL || ctinfo == IP_CT_UNTRACKED) {
		nf_log_buf_add(m, "CT_STATE=UNTRACKED ");
		return;
	}
	state = NF_CT_STATE_BIT(ctinfo);
	if (state == NF_CT_STATE_BIT(IP_CT_ESTABLISHED)) {
		nf_log_buf_add(m, "CT_STATE=ESTABLISHED ");
	} else if (state == NF_CT_STATE_BIT(IP_CT_NEW)) {
		nf_log_buf_add(m, "CT_STATE=NEW ");
	} else if (state == NF_CT_STATE_BIT(IP_CT_RELATED)) {
		nf_log_buf_add(m, "CT_STATE=RELATED ");
	} else {
		nf_log_buf_add(m, "CT_STATE=%d ", state);
	}

	if (CTINFO2DIR(ctinfo) == IP_CT_DIR_REPLY) {
		nf_log_buf_add(m, "CT_DIR=REPLY ");
	} else {
		nf_log_buf_add(m, "CT_DIR=ORIGINAL ");
	}

	nf_log_buf_add(m, "CT_MARK=0x%x ", ct->mark);
}
#endif
			  ...
			  const struct nf_loginfo *loginfo, const char *prefix)
{
	...
	if (loginfo->type == NF_LOG_TYPE_LOG && loginfo->u.log.level != LOGLEVEL_INFO) {
#ifdef CONFIG_IP_ROUTE_CLASSID
		dump_realm(m, loginfo, skb);
#endif
#if IS_ENABLED(CONFIG_NF_CONNTRACK)
		dump_conntrack(m, loginfo, skb);
#endif
	}
}
