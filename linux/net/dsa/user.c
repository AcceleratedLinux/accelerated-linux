...
static int dsa_user_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	case SIOCGMIIREG:
		mii_data = if_mii(ifr);
		if ((mii_data->phy_id >= 32) && ds->ops->get_reg) {
			/* Access to the switch registers */
			ret = ds->ops->get_reg(ds, mii_data->phy_id, mii_data->reg_num);
	...
	case SIOCSMIIREG:
		mii_data = if_mii(ifr);
		if ((mii_data->phy_id >= 32) && ds->ops->set_reg) {
			/* Access to the switch registers */
			return ds->ops->set_reg(ds, mii_data->phy_id, mii_data->reg_num, mii_data->val_in);
		}
		break;
	}
...
{
/*
 * This test causes some DSA switches in DAL to stop working. Noted device are:
 * ConnectEZ8 and 6350-SR. Other devices may not work correctly but haven't been reported.
 */
#if 0
