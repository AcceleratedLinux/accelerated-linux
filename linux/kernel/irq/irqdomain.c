...
static inline bool can_call_select(const struct irq_fwspec *fwspec,
				   enum irq_domain_bus_token bus_token)
{
#ifdef CONFIG_IRQ_DOMAIN_OLD_FWSPEC_SELECT_BEHAVIOR
	(void) bus_token;
	return fwspec->param_count > 0;
#else
	(void) fwspec;
	return bus_token != DOMAIN_BUS_ANY;
#endif
}
					    ...
					    enum irq_domain_bus_token bus_token)
{
	...
	list_for_each_entry(h, &irq_domain_list, link) {
		if (h->ops->select && can_call_select(fwspec, bus_token))
