...
static void ls_pcie_drive_perst(struct ls_pcie *pcie)
{
	if (!pcie->reset)
		return;
...
static int ls_pcie_probe(struct platform_device *pdev)
{
	...
	pcie->reset = devm_gpiod_get_optional(dev, "reset", GPIOD_OUT_HIGH);
	if (IS_ERR(pcie->reset))
		return dev_err_probe(dev, PTR_ERR(pcie->reset),
				     "unable to get reset-gpios\n");
