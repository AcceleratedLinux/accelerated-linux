...
mtk_nand_command_bp(struct mtd_info *mtd, unsigned int command, int column, int page_addr)
{
	...
	case NAND_CMD_PAGEPROG:
		if (g_kCMD.pDataBuf || (0xFF != g_kCMD.au1OOB[nand_badblock_offset])) {
			u8 *pDataBuf = g_kCMD.pDataBuf ? g_kCMD.pDataBuf : nand->data_buf;
...
mtk_nand_read_buf(struct mtd_info *mtd, uint8_t * buf, int len)
{
	...
	if (u4ColAddr < u4PageSize) {
		if ((u4ColAddr == 0) && (len >= u4PageSize)) {
			...
			mtk_nand_exec_read_page(mtd, pkCMD->u4RowAddr, u4PageSize, nand->data_buf, pkCMD->au1OOB);
			memcpy(buf, nand->data_buf + u4ColAddr, len);
		}
...
mtk_nand_read_page_hwecc(struct mtd_info *mtd, struct nand_chip *chip, uint8_t * buf, int oob_required, int page)
{
	...
	mtk_nand_exec_read_page(mtd, page, mtd->writesize, buf, chip->oob_poi);
	return 0;
}
...
read_fact_bbt(struct mtd_info *mtd, unsigned int page)
{
	...
	if (mtk_nand_read_oob_hw(mtd, chip, page)==0)
	{
		...
		if (mtk_nand_exec_read_page(mtd, page, mtd->writesize, chip->data_buf, chip->oob_poi))
		{
			printk("Signature matched and data read!\n");
			memcpy(fact_bbt, chip->data_buf, (bbt_size <= mtd->writesize)? bbt_size:mtd->writesize);
#if 1
{
	int i;
	for (i = 0; i < 4096; i++) {
		if (fact_bbt[i / 4] & (0x3 << ((i % 4) * 2)))
			printk("Bad Block at eraseblock %d\n", i);
	}
}
#endif
			return 0;
...
mtk_nand_probe(struct platform_device *pdev)
{
	...
	*/
	...
	nand_chip->data_buf = kzalloc(mtd->writesize + mtd->oobsize, GFP_KERNEL);
	nand_chip->ecc.calc_buf = kzalloc(mtd->oobsize, GFP_KERNEL);
	nand_chip->ecc.code_buf = kzalloc(mtd->oobsize, GFP_KERNEL);
	if (!nand_chip->data_buf || !nand_chip->ecc.calc_buf || !nand_chip->ecc.code_buf)
		return -ENOMEM;

	nand_chip->oob_poi = nand_chip->data_buf + mtd->writesize;
...
out:
	...
	kfree(nand_chip->data_buf);
	kfree(nand_chip->ecc.calc_buf);
	kfree(nand_chip->ecc.code_buf);
	kfree(host);
	nand_disable_clock();
	return err;
}
