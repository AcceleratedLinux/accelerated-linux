...
nettel_probe(int ram, unsigned long addr, int size, int buswidth)
{
	...
	mymtd->_point = nettel_point;
	mymtd->priv = map_ptr;

	if (ram) {
		ram_mtdinfo = mymtd;
		mtd_device_register(mymtd, nettel_romfs, NB_OF(nettel_romfs));
		return(0);
	}
	...
	switch (size) {
	case SIZE_128K:
		mtd_device_register(mymtd, nettel_128k, NB_OF(nettel_128k));
		break;
	case SIZE_1MB:
		mtd_device_register(mymtd, nettel_1mb, NB_OF(nettel_1mb));
		break;
	case SIZE_2MB:
		mtd_device_register(mymtd, nettel_2mb, NB_OF(nettel_2mb));
		break;
	case SIZE_4MB:
		mtd_device_register(mymtd, nettel_4mb, NB_OF(nettel_4mb));
		break;
	case SIZE_8MB:
		mtd_device_register(mymtd, nettel_8mb, NB_OF(nettel_8mb));
		break;
	case SIZE_16MB:
		mtd_device_register(mymtd, nettel_16mb, NB_OF(nettel_16mb));
		break;
	}
...
int __init nettel_mtd_init(void)
{
	...
	if (0 != nettel_probe(1, (unsigned long) __bss_stop,
	    PAGE_ALIGN(* (unsigned long *)(((unsigned long) __bss_stop) + 8)), 4))
...
#else
	{
		...
		start_area = (unsigned long) __bss_stop;

		if (strncmp((char *) start_area, "-rom1fs-", 8) != 0) {
			mtd = get_mtd_named("Image");
			if (mtd && mtd->_point) {
				if ((*mtd->_point)(mtd, 0, mtd->size, &len, &sp, NULL) == 0) {
					ep = sp + len;
