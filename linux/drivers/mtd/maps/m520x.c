...
m520x_probe(int type, unsigned long addr, int size, int bankwidth)
{
	...
	mymtd->_point = m520x_point;
	mymtd->priv   = map_ptr;

	if (type) {
		ram_mtdinfo = mymtd;
		mtd_device_register(mymtd, m520x_romfs, NB_OF(m520x_romfs));
	} else {
		flash_mtdinfo = mymtd;
		mtd_device_register(mymtd, m520x_partitions, 
...
int __init init_m520x(void)
{
	...
	if (0 != m520x_probe( 1, (unsigned long) __bss_stop,
			      PAGE_ALIGN(*(unsigned long *)(__bss_stop + 8)), 4))
...
#else
	{
		...
		if (strncmp((char *) start_area, "-rom1fs-", 8) != 0) {
			...
			if (mtd && mtd->_point) {
				if ((*mtd->_point)(mtd, 0, mtd->size, &len, &sp) == 0){
					ep = sp + len;
