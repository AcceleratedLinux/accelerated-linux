...
static struct allowed_path allowed_paths[] = {
#ifdef CONFIG_USB_AWUSB_HUB_WHITELIST
	{ 1, "2" },
	{ 2, "2" },
#else /* ConnectEZ8/16/32 */
	{ 6, "1" }, /* SD card reader */
	{ 3, "1" }, /* Modem */
	{ 4, "1" }, /* Modem */
#endif
...
static bool is_whitelist_path(struct usb_device *udev)
{
	int i;

	for (i = 0; i < (sizeof(allowed_paths) / sizeof(allowed_paths[0])); i++) {
		if (allowed_paths[i].busnum == udev->bus->busnum &&
		    !strncmp(allowed_paths[i].devpath, udev->devpath, sizeof(allowed_paths[i].devpath))) {
			dev_dbg(&udev->dev, "Matched allowed path: bus: %d, devpath: %s\n", udev->bus->busnum, udev->devpath);
			return true;
		}
	}
...
#endif // CONFIG_USB_AWUSB_DEFAULT_CLAIM_PORT
...
{
	int retval;
#ifdef CONFIG_USB_XHCI_AWUSB_EXTEND_CMD_RING_TIMEOUT
	unsigned int timeout_ms = 300000;
#else
		...
		struct usb_device_descriptor *buf, int size, bool first_time)
{
	...
	for (i = 0; i < GET_MAXPACKET0_TRIES; ++i) {
		if (udev->quirks & USB_QUIRK_DELAY_DESC_READ)
			msleep(100);
