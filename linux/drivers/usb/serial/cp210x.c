...
#ifdef CONFIG_SERIAL_CP210X_DIGI_REALPORT_EXTENSIONS
...
#ifdef CONFIG_SERIAL_CP210X_DIGI_REALPORT_EXTENSIONS
static int cp210x_send_immediate(struct usb_serial_port *port, unsigned char);
static int cp210x_ioctl(struct tty_struct *tty,
			unsigned int cmd, unsigned long arg);
static int cp210x_realport_set_flow_control(struct tty_struct *tty,
											struct usb_serial_port *port,
											unsigned int sigs);
static int cp210x_dump_state(struct tty_struct *tty, struct usb_serial_port *port);
static void cp210x_flush_buffer(struct tty_struct *tty);
#endif
...
#ifdef CONFIG_SERIAL_CP210X_DIGI_REALPORT_EXTENSIONS
	.ioctl			= cp210x_ioctl,
	.start			= usb_serial_generic_start,
	.stop			= usb_serial_generic_stop,
	.flush_buffer	= cp210x_flush_buffer,
#endif
};
...
#ifdef CONFIG_SERIAL_CP210X_DIGI_REALPORT_EXTENSIONS
static int cp210x_realport_set_flow_control(struct tty_struct *tty,
							struct usb_serial_port *port, unsigned int sigs)
{
	struct cp210x_port_private *port_priv = usb_get_serial_port_data(port);
	struct cp210x_flow_ctl flow_ctl;
	u32 control_hs=0, flow_replace;
	int result;

	dev_dbg(&port->dev, "%s sigs=0x%X", __func__, sigs);

	mutex_lock(&port_priv->mutex);

	if (sigs & TIOCM_DTR)
	{
		dev_dbg(&port->dev, "%s DTR flow control", __func__);
		control_hs |= CP210X_SERIAL_DTR_FLOW_CTL;
		port_priv->cdtrdts = true;
	}
	else
	{
		if (port_priv->dtr)
			control_hs |= CP210X_SERIAL_DTR_ACTIVE;
		else
			control_hs |= CP210X_SERIAL_DTR_INACTIVE;
		port_priv->cdtrdts = false;
	}

	if (sigs & TIOCM_CTS)
	{
		dev_dbg(&port->dev, "%s CTS flow control", __func__);
		control_hs |= CP210X_SERIAL_CTS_HANDSHAKE;
	}

	if (sigs & TIOCM_DSR)
	{
		dev_dbg(&port->dev, "%s DSR flow control", __func__);
		control_hs |= CP210X_SERIAL_DSR_HANDSHAKE;
	}

	if (sigs & TIOCM_CD)
		control_hs |= CP210X_SERIAL_DCD_HANDSHAKE;

	result = cp210x_read_reg_block(port, CP210X_GET_FLOW, &flow_ctl, sizeof(flow_ctl));
	if (result) {
		dev_err(&port->dev, "%s CP210X_GET_FLOW failed", __func__);
		goto done;
	}

	flow_replace = le32_to_cpu(flow_ctl.ulFlowReplace);

	flow_replace &= ~CP210X_SERIAL_RTS_MASK;
	if (sigs & TIOCM_RTS)
	{
		dev_dbg(&port->dev, "%s RTS flow control", __func__);
		flow_replace |= CP210X_SERIAL_RTS_FLOW_CTL;
		port_priv->crtscts = true;
	}
	else
	{
		if (port_priv->rts)
			flow_replace |= CP210X_SERIAL_RTS_ACTIVE;
		else
			flow_replace |= CP210X_SERIAL_RTS_INACTIVE;
		port_priv->crtscts = false;
	}

	dev_dbg(&port->dev, "%s - control handshake = 0x%08x, flow replace = "
						"0x%08x\n", __func__, control_hs, flow_replace);

	flow_ctl.ulControlHandshake = cpu_to_le32(control_hs);
	flow_ctl.ulFlowReplace = cpu_to_le32(flow_replace);

	result = cp210x_write_reg_block(port, CP210X_SET_FLOW, &flow_ctl, sizeof(flow_ctl));

#ifdef DEBUG
	cp210x_dump_state(tty, port);
#endif
		...
		struct usb_serial_port *port, struct ktermios *old_termios)
{
	...
	dev_dbg(&port->dev, "%s CP210X_SET_FLOW- ctrl = 0x%02x, flow = 0x%02x\n", __func__,
			ctl_hs, flow_repl);
		...
		unsigned int set, unsigned int clear)
{
	...
	dev_dbg(&port->dev, "%s set=%X clear=%X\n", __func__, set, clear);

	mutex_lock(&port_priv->mutex);

 	if (set & TIOCM_RTS) {
		dev_dbg(&port->dev, "%s set RTS\n", __func__);
		port_priv->rts = true;
		control |= CONTROL_RTS;
		control |= CONTROL_WRITE_RTS;
	}
	if (set & TIOCM_DTR) {
		dev_dbg(&port->dev, "%s set DTR\n", __func__);
		port_priv->dtr = true;
		control |= CONTROL_DTR;
	...
	if (clear & TIOCM_RTS) {
		dev_dbg(&port->dev, "%s clear RTS\n", __func__);
		port_priv->rts = false;
		control &= ~CONTROL_RTS;
		control |= CONTROL_WRITE_RTS;
	}
	if (clear & TIOCM_DTR) {
		dev_dbg(&port->dev, "%s clear DTR\n", __func__);
		port_priv->dtr = false;
		control &= ~CONTROL_DTR;
		control |= CONTROL_WRITE_DTR;
	}
	 ...
	 */
#ifdef CONFIG_SERIAL_CP210X_DIGI_REALPORT_EXTENSIONS
	if ((port_priv->crtscts || port_priv->cdtrdts) && (control & CONTROL_WRITE_RTS)) {
#else
...
static int cp210x_send_immediate(struct usb_serial_port *port, unsigned char immed)
{
	int result = 0;
	struct cp210x_port_private *port_priv = usb_get_serial_port_data(port);

	result = cp210x_write_u16_reg(port, CP210X_IMM_CHAR, immed);
	if (result)
		dev_err(&port->dev, "%s - Unable to send immediate char\n", __func__);

	return result;
}

static int cp210x_realport_get_hw_stats(struct tty_struct *tty,
										struct usb_serial_port *port,
										struct realport_hw_stats *stats)
{

	memset(stats, 0, sizeof(struct realport_hw_stats));

	stats->estat = 0;
	stats->tiocser_temt = true;
	stats->tiocmget = cp210x_tiocmget(tty);

	cp210x_get_tx_queue_byte_count(port, &stats->tiocoutq);
	stats->tiocoutq += usb_serial_generic_chars_in_buffer(tty);
...
static int cp210x_dump_serial_status(struct tty_struct *tty, struct usb_serial_port *port)
{
	struct usb_serial *serial = port->serial;
	struct cp210x_port_private *port_priv = usb_get_serial_port_data(port);
	struct cp210x_comm_status *sts;
	...
	dev_err(&port->dev, "%s   Table-8\n", __func__);
	dev_err(&port->dev, "%s   Errors = %08x\n", __func__, sts->ulErrors);
	dev_err(&port->dev, "%s   HoldReasons = %08x\n", __func__, sts->ulHoldReasons);
	dev_err(&port->dev, "%s   Bytes In Queues (In/Out) = x%x / x%x\n", __func__,
						sts->ulAmountInOutQueue, sts->ulAmountInOutQueue);
	dev_err(&port->dev, "%s   EOF Rcvd = %d  Imm. Pending: %x\n", __func__,
						sts->bEofReceived, sts->bWaitForImmediate);
done:
	kfree(sts);
	return result;
}

static int cp210x_dump_flow_settings(struct tty_struct *tty, struct usb_serial_port *port)
{
	struct cp210x_flow_ctl flow_ctl;
	int result = 0;
	u32 control_hs=0, flow_replace;

	if (cp210x_read_reg_block(port, CP210X_GET_FLOW, &flow_ctl, sizeof(flow_ctl)))
	{
		dev_err(&port->dev, "%s - Failed to Read_reg_block\n", __func__);
		return -EIO;
	}

	flow_replace = le32_to_cpu(flow_ctl.ulFlowReplace);
	control_hs = le32_to_cpu(flow_ctl.ulControlHandshake);

	dev_err(&port->dev, "%s   Ctrl Handshake = %08x\n", __func__, flow_ctl.ulControlHandshake);
	dev_err(&port->dev, "%s: DTR_ACTIVE    %s\n", __func__, (control_hs & CP210X_SERIAL_DTR_ACTIVE) ? "ON" : "OFF");
	dev_err(&port->dev, "%s: DTR_FLOW_CTL  %s\n", __func__, (control_hs & CP210X_SERIAL_DTR_FLOW_CTL) ? "ON" : "OFF");
	dev_err(&port->dev, "%s: CTS_HANDSHAKE %s\n", __func__, (control_hs & CP210X_SERIAL_CTS_HANDSHAKE) ? "ON" : "OFF");
	dev_err(&port->dev, "%s: DSR_HANDSHAKE %s\n", __func__, (control_hs & CP210X_SERIAL_DSR_HANDSHAKE) ? "ON" : "OFF");
	dev_err(&port->dev, "%s: DCD_HANDSHAKE %s\n", __func__, (control_hs & CP210X_SERIAL_DCD_HANDSHAKE) ? "ON" : "OFF");

	dev_err(&port->dev, "%s: Flow Replace   = %08x\n", __func__, flow_replace);
	dev_err(&port->dev, "%s: Xon/Xoff Limit = %08x/%08x\n", __func__, flow_ctl.ulXoffLimit, flow_ctl.ulXoffLimit);
	return result;
}

static int cp210x_dump_line_ctl(struct tty_struct *tty, struct usb_serial_port *port)
{
	__le16	line_ctrl;
	int result = 0;

	if (cp210x_read_reg_block(port, CP210X_GET_LINE_CTL, &line_ctrl, sizeof(line_ctrl)))
	{
		dev_err(&port->dev, "%s - Failed to Read_reg_block\n", __func__);
		return -EIO;
	}

	dev_err(&port->dev, "%s: Line control=0x%hx \n", __func__, le16_to_cpu(line_ctrl));
	return result;
}


static int cp210x_dump_chars(struct tty_struct *tty, struct usb_serial_port *port)
{
	struct cp210x_special_chars chars;

	int result = 0;

	if (cp210x_read_reg_block(port, CP210X_GET_CHARS, &chars, sizeof(chars)))
	{
		dev_err(&port->dev, "%s - Failed to Read_reg_block\n", __func__);
		return -EIO;
	}

	dev_err(&port->dev, "%s: EofChar=%u\n", __func__, (unsigned int)chars.bEofChar);
	dev_err(&port->dev, "%s: bErrorChar=%u\n", __func__, (unsigned int)chars.bErrorChar);
	dev_err(&port->dev, "%s: bBreakChar=%u\n", __func__, (unsigned int)chars.bBreakChar);
	dev_err(&port->dev, "%s: bEventChar=%u\n", __func__, (unsigned int)chars.bEventChar);
	dev_err(&port->dev, "%s: bXonChar=%u\n", __func__, (unsigned int)chars.bXonChar);
	dev_err(&port->dev, "%s: bXoffChar=%u\n", __func__, (unsigned int)chars.bXoffChar);
	return result;
}
static int cp210x_dump_state(struct tty_struct *tty, struct usb_serial_port *port)
{
	int result = 0;
	int sigs = cp210x_tiocmget(tty);

	dev_err(&port->dev, "------------------------------------------------------------");
	dev_err(&port->dev, "%s: RTS  %s\n", __func__, (sigs & TIOCM_RTS) ? "ON" : "OFF");
	dev_err(&port->dev, "%s: DTR  %s\n", __func__, (sigs & TIOCM_DTR) ? "ON" : "OFF");
	dev_err(&port->dev, "%s: CTS  %s\n", __func__, (sigs & TIOCM_CTS) ? "ON" : "OFF");
	dev_err(&port->dev, "%s: DSR  %s\n", __func__, (sigs & TIOCM_DSR) ? "ON" : "OFF");
	dev_err(&port->dev, "%s: RING %s\n", __func__, (sigs & TIOCM_RNG) ? "ON" : "OFF");
	dev_err(&port->dev, "%s: CD   %s\n", __func__, (sigs & TIOCM_CD) ? "ON" : "OFF");

	cp210x_dump_serial_status(tty, port);
	cp210x_dump_flow_settings(tty, port);
	cp210x_dump_line_ctl(tty, port);
	cp210x_dump_chars(tty, port);
						...
						unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
		case DIGI_REALPORT_SEND_IMMEDIATE: {
			...
			if (cp210x_send_immediate(port, immed))
				result = -EIO;
		...
		case DIGI_REALPORT_SET_FLOW_SIGNALS: {
			...
			if (cp210x_realport_set_flow_control(tty, port, sigs))
				result = -EIO;
		...
		case DIGI_REALPORT_GET_HW_STATS: {
			struct realport_hw_stats stats;
			if (cp210x_realport_get_hw_stats(tty, port, &stats))
				return -EIO;

			return copy_to_user((struct realport_hw_stats __user*) arg, &stats, sizeof(stats));
		}
		case DIGI_REALPORT_STOP_BREAK: {
			cp210x_break_ctl(tty, BREAK_OFF);
			return result;
		}
		case DIGI_REALPORT_START_BREAK: {
			cp210x_break_ctl(tty, BREAK_ON);
			return result;
		}
		case DIGI_REALPORT_DUMP_DRIVER_STATE: {
			return cp210x_dump_state(tty, port);
		}
	}
	return -ENOIOCTLCMD;
}

void cp210x_flush_buffer(struct tty_struct *tty) {
	struct usb_serial_port *port = tty->driver_data;

	usb_serial_generic_flush_buffer(tty);
	cp210x_write_u16_reg(port, CP210X_PURGE, PURGE_TX);
}
