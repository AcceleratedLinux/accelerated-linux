...
void usb_serial_generic_stop(struct tty_struct *tty)
{
	struct usb_serial_port *port = tty->driver_data;
	dev_err(&port->dev, "usb_serial_generic_stop\n");
	set_bit(USB_SERIAL_WRITE_BUSY, &port->flags);
}
EXPORT_SYMBOL_GPL(usb_serial_generic_stop);

void usb_serial_generic_start(struct tty_struct *tty)
{
	struct usb_serial_port *port = tty->driver_data;
	dev_err(&port->dev, "usb_serial_generic_start\n");
	clear_bit(USB_SERIAL_WRITE_BUSY, &port->flags);
	usb_serial_generic_write_start(port, GFP_ATOMIC);
}
EXPORT_SYMBOL_GPL(usb_serial_generic_start);

void usb_serial_generic_flush_buffer(struct tty_struct *tty)
{
	struct usb_serial_port *port = tty->driver_data;
	unsigned long flags;
	int i;
...
EXPORT_SYMBOL_GPL(usb_serial_generic_flush_buffer);
