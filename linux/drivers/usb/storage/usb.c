...
#ifdef CONFIG_SG8200
	/*
	 * On 8200 devices, the usb numbering can be inconsistent. This means we can't
	 * guarantee the boot device is on /dev/sda1. As we don't use initrd, we can't
	 * boot with root=UUID=.... or root=LABEL=..., and we need gst to use
	 * root=PARTUUID=...
	 *
	 * This patch will disable any usb mass storage device not on the correct bus.
	 * The internal usb is on bus 1 for kernel 4.4, and bus 3 for kernel 4.13 - I
	 * believe this is due to the order that drivers are loaded for the hubs. If the
	 * boot fails in the future, we need to check which bus the internal usb is on
	 * for the specific kernel.
	 *
	 * The following code disables all busses except the internal one for 10 seconds.
	 * This is easily long enough to boot and detect the internal usb as /dev/sda. This
	 * is needed for both the big and little kernels (8200.bin and 8200-kboot.bin)
	 *
	 */
	struct usb_device *udev = interface_to_usbdev(intf);
	unsigned int uptime = jiffies + 5*60*HZ; // 5 minutes offset - should wrap back to positive

	dev_info(&intf->dev, "USB Mass Storage device detected\n");

	if( uptime < 10*HZ && udev->bus->busnum != 3 ) // hard coded bus 3 for kernel 4.13
	{
		dev_info(&intf->dev, "USB Mass Storage ignored during boot.\n");
		return -EACCES;
	}
#endif
