		...
		struct xhci_event_cmd *event)
{
...
#ifdef CONFIG_USB_XHCI_AWUSB_EXTEND_CMD_RING_TIMEOUT
	{
		...
		if (ktime_to_ms(cur - cmd->queued_time) > 5000)
			xhci_warn(xhci, "CMD(%llu) completion: %llu ms\n",
			                TRB_FIELD_TO_TYPE(le32_to_cpu(cmd_trb->generic.field[3])),
				            ktime_to_ms(cur - cmd->queued_time));
	}
#endif
