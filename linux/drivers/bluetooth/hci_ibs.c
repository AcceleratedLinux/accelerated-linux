...
{
	struct ibs_struct *ibs = from_timer(ibs, t, tx_idle_timer);
	struct hci_uart *hu = ibs->ibs_hu;
	unsigned long flags;
...
static int ibs_open(struct hci_uart *hu)
{
	...
	timer_setup(&ibs->wake_retrans_timer, hci_ibs_wake_retrans_timeout, 0);
	timer_setup(&ibs->tx_idle_timer, hci_ibs_tx_idle_timeout, 0);
