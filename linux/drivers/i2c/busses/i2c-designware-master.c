...
/* Change I2C bus speed (in kHz). The following options are possible:
 * 0:		means keep default
 * 100:		100kHz
 * 400:		400kHz
 * 3400:	3.4MHz
 *
 * If parameter does not match, it will be rounded down to the nearest option.
 */
static unsigned int bus_speed = 0;
module_param(bus_speed, uint, S_IRUGO);
MODULE_PARM_DESC(bus_speed, "Bus Speed in kHz (0(default) / 100 / 400 / 3400)");
...
int i2c_dw_probe_master(struct dw_i2c_dev *dev)
{
	...
	/* Change bus speed per module parameter */
	if (bus_speed == 0) {
		/* Do nothing, keep existing */
	}
	else if (bus_speed < 400) {
		bus_speed = 100;
		dev->master_cfg &= ~DW_IC_CON_SPEED_MASK;
		dev->master_cfg |= DW_IC_CON_SPEED_STD;
	}
	else if (bus_speed < 3400) {
		bus_speed = 400;
		dev->master_cfg &= ~DW_IC_CON_SPEED_MASK;
		dev->master_cfg |= DW_IC_CON_SPEED_FAST;
	}
