...
#ifdef CONFIG_SERIAL_STM32_DIGI_REALPORT_EXTENSIONS
...
static void stm32_usart_transmit_chars(struct uart_port *port)
{
	...
	if (!stm32_port->hw_flow_control &&
	    ...
	    (port->x_char ||
#ifdef CONFIG_SERIAL_STM32_DIGI_REALPORT_EXTENSIONS
	    (stm32_port->rp_send_immediate >= 0) ||
	    (stm32_port->send_nul) ||
#endif
...
#ifdef CONFIG_SERIAL_STM32_DIGI_REALPORT_EXTENSIONS
	if (stm32_port->rp_send_immediate >= 0) {
		...
		writel_relaxed((unsigned char) stm32_port->rp_send_immediate,
				port->membase + ofs->tdr);
                stm32_port->rp_send_immediate = -1;
		port->icount.tx++;
	...
	if (stm32_port->send_nul) {
		...
		writel_relaxed(0, port->membase + ofs->tdr);
                stm32_port->send_nul = false;
		port->icount.tx++;
...
static void stm32_usart_set_mctrl(struct uart_port *port, unsigned int mctrl)
{
	...
	/*
	 * If stm32_port->gpio_rts is set, the port is in a non-hardware-flow
	 * control mode.
	 * This allows for direct manipulation of RTS as needed.
	 */
	if (stm32_port->gpio_rts)
		gpiod_set_value(stm32_port->gpio_rts, mctrl & TIOCM_RTS);
}
...
#ifdef CONFIG_SERIAL_STM32_DIGI_REALPORT_EXTENSIONS
	struct stm32_port *stm32_port = to_stm32_port(port);
	if (kfifo_is_empty(&tport->xmit_fifo) && !port->x_char && (stm32_port->rp_send_immediate < 0) && !stm32_port->send_nul) {
#else
...
static void stm32_switch_pinctrl_modes(struct uart_port *port, int mode)
{
	struct stm32_port *stm32_port = to_stm32_port(port);
	struct gpio_desc *gpiod;

	if (mode < 0 || mode >= MAX_PINCTRL_STATES || !stm32_port || !stm32_port->states[mode])
		return;

	/*
	 * Don't switch modes while in Long Break.
	 * Long Break needs to be released before
	 * switching modes is allowed.
	 */
	if (stm32_port->in_break)
		return;

	switch (mode) {
	case DEFAULT:
		/* If hw flow-control selected, stop handling RTS manually */
		if (stm32_port->gpio_rts) {
			devm_gpiod_put(port->dev, stm32_port->gpio_rts);
			stm32_port->gpio_rts = NULL;
		}
		pinctrl_select_state(stm32_port->pinctrl,
				     stm32_port->states[DEFAULT]);
		break;
	case NO_HW_FLOWCTRL:
		/* RTS is not controlled by hwflow, we can control it instead */
		pinctrl_select_state(stm32_port->pinctrl,
				     stm32_port->states[NO_HW_FLOWCTRL]);
		/* Map the rts gpio pin in */
		if (!stm32_port->gpio_rts) {
			gpiod = devm_gpiod_get(port->dev, "digi,rts",
					       STM32_DEFAULT_RTS_GPIOD_STATE);
			if (!IS_ERR(gpiod)) {
				gpiod_set_consumer_name(gpiod,
						port->dev->of_node->name);
				stm32_port->gpio_rts = gpiod;
			}
		}
		break;
	case LONG_BREAK:
		/*
		 * 'Long Break' mode purposely does not touch RTS.
		 * It remains seperate, so that it can be driven high/low
		 * if needed.
		 */
		pinctrl_select_state(stm32_port->pinctrl,
				     stm32_port->states[LONG_BREAK]);
		/* Map the rts gpio pin in */
		if (!stm32_port->gpio_rts) {
			gpiod = devm_gpiod_get(port->dev, "digi,rts",
					       STM32_DEFAULT_RTS_GPIOD_STATE);
		...
		break;
	default:
		dev_err(port->dev, "Unknown '%d' mode\n", mode);
		break;
	}
}
...
static void stm32_usart_break_ctl(struct uart_port *port, int break_state)
{
	...
	if (stm32_port->states[LONG_BREAK])
		long_break = true;
	...
	if (break_state) {
		if (long_break) {
			stm32_switch_pinctrl_modes(port, LONG_BREAK);
			stm32_port->in_break = true;
		} else {
			stm32_usart_set_bits(port, ofs->rqr, USART_RQR_SBKRQ);
		}
	} else {
		if (long_break) {
			stm32_port->in_break = false;

			/* Look up what flow control we should move to */
			if (tty->termios.c_cflag & CRTSCTS)
				stm32_switch_pinctrl_modes(port, DEFAULT);
			else
				stm32_switch_pinctrl_modes(port, NO_HW_FLOWCTRL);
		} else {
...
static int stm32_usart_realport_set_flow_control(struct uart_port *port, unsigned int sigs)
{
	...
	memcpy(&old_termios, &tty->termios, sizeof(struct ktermios));

	spin_lock_irqsave(&port->lock, flags);

	if (sigs & TIOCM_RTS) {
		dev_dbg(port->dev, "%s - Turn on RTS/CTS flow control", __func__);
		tty->termios.c_cflag |= (CRTSCTS);
	}
	else {
		dev_dbg(port->dev, "%s - Turn off RTS/CTS flow control", __func__);
		tty->termios.c_cflag &= ~(CRTSCTS);
	}

	/*
	 * Currently, we do not support DTR/DTS/DCD flow control.
	 * TODO: Need to investigate chip specs to see if it could be
	 */
	stm32port->cdtrdsr = false;

	if (sigs & TIOCM_DTR)
	{
		//stm32port->cdtrdsr = true;
		ret = -EINVAL;
	...
	{
		ret = -EINVAL;
		dev_dbg(port->dev, "%s - DCD flow control not supported", __func__);
	}

	spin_unlock_irqrestore(&port->lock, flags);

	if (memcmp(&tty->termios, &old_termios, sizeof(struct ktermios))) {
		if (port->ops->set_termios) {
			port->ops->set_termios(port, &tty->termios, &old_termios);
		}
		else {
			stm32_usart_set_termios(port, &tty->termios, &old_termios);
		}
	}
...
static int stm32_usart_send_immediate(struct stm32_port *stm32port, unsigned char immed)
{
	int ret;
	unsigned long flags;

	spin_lock_irqsave(&stm32port->port.lock, flags);
	if (stm32port->rp_send_immediate < 0) {
		stm32port->rp_send_immediate = immed;
		stm32_usart_start_tx(&stm32port->port);
		ret = 0;
	} else
		ret = -EAGAIN;
	spin_unlock_irqrestore(&stm32port->port.lock, flags);
	return ret;
}

static void stm32_usart_get_hw_stats(struct uart_port *port, struct realport_hw_stats *stats)
{
	struct stm32_port *stm32port = container_of(port,
			struct stm32_port, port);
	unsigned long flags;
	...
	spin_lock_irqsave(&port->lock, flags);
	stats->tiocoutq = kfifo_len(&port->state->port.xmit_fifo);
	stats->tiocmget = stm32_usart_get_mctrl(port);
	...
	stats->estat = stm32port->estat;
	if (stm32port->rp_send_immediate >= 0)
		stats->estat |= RP_TXI;
	if (port->x_char || stm32port->send_nul)
		stats->estat |= RP_TXF;

	/* Grab the output gpios as well */
	mctrl = TIOCM_RTS | TIOCM_DTR;
	stats->tiocmget |= mctrl_gpio_get_outputs(stm32port->gpios, &mctrl);

	spin_unlock_irqrestore(&port->lock, flags);
}

static int stm32_usart_ioctl(struct uart_port *port, unsigned int cmd, unsigned long arg)
{
	struct stm32_port *stm32port = container_of(port,
			struct stm32_port, port);
	int ret = 0;
	...
	switch(cmd) {
		case DIGI_REALPORT_SEND_IMMEDIATE: {
			...
			if (copy_from_user(&immed, (unsigned char __user *) arg, sizeof(immed)))
				return -EFAULT;
			return stm32_usart_send_immediate(stm32port, immed);
		}
		case DIGI_REALPORT_SET_FLOW_SIGNALS: {
			...
			if (copy_from_user(&sigs,(unsigned int __user *) arg, sizeof(sigs)))
			 ...
			 if (stm32_usart_realport_set_flow_control(port, sigs))
				ret = -EIO;
		...
		case DIGI_REALPORT_GET_HW_STATS: {
			...
			stm32_usart_get_hw_stats(port, &stats);
		...
		case DIGI_REALPORT_START_BREAK: {
			...
			stm32_usart_break_ctl(port, 1);
			stm32port->estat |= RP_TXB;
			stm32port->break_pending = true;
			return ret;
		}
		case DIGI_REALPORT_STOP_BREAK: {
			stm32_usart_break_ctl(port, 0);
			stm32port->estat &= ~RP_TXB;
			stm32port->break_pending = false;
			return ret;
		}
		case DIGI_REALPORT_DUMP_DRIVER_STATE: {
			/* Perhaps one day we will add support for this */
			return -ENOIOCTLCMD;
		}
	}
	return -ENOIOCTLCMD;
...
static void stm32_usart_send_xchar(struct uart_port *port, char ch)
{
	unsigned long flags;
	struct tty_struct *tty = port->state->port.tty;
	...
	port->x_char = ch;
	if (ch)
		stm32_usart_start_tx(port);
	else if (tty->termios.c_line == N_REALPORT) {
		struct stm32_port *stm32port = container_of(port,
					struct stm32_port, port);
		stm32port->send_nul = true;
		stm32_usart_start_tx(port);
	}
	spin_unlock_irqrestore(&port->lock, flags);
}
...
static const struct uart_ops stm32_uart_ops = {
	...
	.send_xchar	= stm32_usart_send_xchar,
#endif
#if defined(CONFIG_CONSOLE_POLL)
				 ...
				 struct platform_device *pdev)
{
	...
	if (of_get_property(pdev->dev.of_node, "digi,no-hw-flowctrl", NULL) ||
		of_get_property(pdev->dev.of_node, "digi,long-break", NULL)) {

		stm32port->in_break = false;

		stm32port->pinctrl = devm_pinctrl_get(&pdev->dev);
		if (IS_ERR(stm32port->pinctrl)) {
			ret = PTR_ERR(stm32port->pinctrl);
			dev_err(&pdev->dev, "Failed to get Pinctrl: %d\n", ret);
			ret = -EINVAL;
			goto err_clk;
		}

		stm32port->states[DEFAULT] =
			pinctrl_lookup_state(stm32port->pinctrl, "default");
		if (IS_ERR(stm32port->states[DEFAULT])) {
			ret = PTR_ERR(stm32port->states[DEFAULT]);
			dev_err(&pdev->dev,
		...
		if (of_get_property(pdev->dev.of_node, "digi,no-hw-flowctrl", NULL)) {
			stm32port->states[NO_HW_FLOWCTRL] =
			pinctrl_lookup_state(stm32port->pinctrl, "no-hw-flowctrl");
			if (IS_ERR(stm32port->states[NO_HW_FLOWCTRL])) {
				ret = PTR_ERR(stm32port->states[NO_HW_FLOWCTRL]);
				stm32port->states[NO_HW_FLOWCTRL] = NULL;
			}
		}
		else {
			stm32port->states[NO_HW_FLOWCTRL] = NULL;
		...
		/* "break" state is optional */
		if (of_get_property(pdev->dev.of_node, "digi,long-break", NULL)) {
			stm32port->states[LONG_BREAK] =
			pinctrl_lookup_state(stm32port->pinctrl, "gpio");
			if (IS_ERR(stm32port->states[LONG_BREAK])) {
				ret = PTR_ERR(stm32port->states[LONG_BREAK]);
				stm32port->states[LONG_BREAK] = NULL;
			}
		}
		else {
			stm32port->states[LONG_BREAK] = NULL;
