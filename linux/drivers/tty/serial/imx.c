...
static irqreturn_t imx_dcdint(int irq, void *dev_id)
{
	struct imx_port *sport = dev_id;
	struct uart_port *p = &sport->port;
	unsigned long flags;

        spin_lock_irqsave(&p->lock, flags);
        uart_handle_dcd_change(p, gpio_get_value(sport->have_dcd_gpio) ? 0 : 1);
			     ...
			     struct platform_device *pdev)
{
	...
	if (of_get_property(np, "fsl,uart-has-dcd-gpio", NULL)) {
		...
		if (devm_request_irq(&pdev->dev, irq, imx_dcdint, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, dev_name(&pdev->dev), sport))
			pr_err("failed to register IRQ %d for UART DCD\n", irq);
		else
			pr_info("registered IRQ %d for UART DCD action\n", irq);
	}
