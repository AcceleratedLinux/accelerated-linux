...
static bool start_tx_rts_toggle(struct uart_port *port);
...
static inline void __stop_tx(struct uart_8250_port *p)
{
	...
	if (em485) {
		...
		if ((lsr & UART_LSR_BOTH_EMPTY) != UART_LSR_BOTH_EMPTY) {
			if (lsr & UART_LSR_THRE) {
...
#ifdef CONFIG_SERIAL_8250_PERICOM_DIGI_EXTENSIONS
	if (up->rp_send_immediate < 0 && !port->x_char && kfifo_is_empty(&port->state->port.xmit_fifo))
#else
...
static void serial_8250_process_char(struct uart_8250_port *up, u8 ch, u16 lsr)
{
	struct uart_port *port = &up->port;
	u8 flag = TTY_NORMAL;
...
#ifdef CONFIG_SERIAL_8250_PERICOM_DIGI_EXTENSIONS
static void serial8250_do_modem_status(struct uart_8250_port *up, unsigned char status)
{
	struct uart_port *port = &up->port;
