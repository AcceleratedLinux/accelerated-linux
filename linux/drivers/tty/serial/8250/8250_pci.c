...
#define PERICOM_DRIVER_SEL            0xb4
#define PERICOM_DRIVER_SEL_PORT_SHIFT 0x4
#define PERICOM_DRIVER_SEL_PORT_MASK  0xf
#define PERICOM_DRIVER_SEL_232        0x0
#define PERICOM_DRIVER_SEL_422        0x1
#define PERICOM_DRIVER_SEL_485_4W     0xb
#define PERICOM_DRIVER_SEL_485_2W     0xf
/* The 4 port decided to store port 4 at the end of the 32-bit 0xb4 register instead of in a linear fashion like the 8-port */
#define DRIVER_SEL_PORT_SHIFT(x, y) (x == 0x3 && y == PCI_DEVICE_ID_PERICOM_PI7C9X7954 ? 28 : x * PERICOM_DRIVER_SEL_PORT_SHIFT)
static ssize_t uart_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct pci_dev *pdev = container_of(dev, struct pci_dev, dev);
	struct serial_private *priv = pci_get_drvdata(pdev);
	const struct pciserial_board *board = priv->board;
	...
	if (i >= sizeof(tmp_buf) - 1) {
		dev_err(&pdev->dev, "Invalid uart index.");
		return -EINVAL;
	}

	memcpy(tmp_buf, buf, i);
	tmp_buf[i++] = '\0';
	if (kstrtol(tmp_buf, 10, &idx)) {
		dev_err(&pdev->dev, "Invalid uart index.");
		return -EINVAL;
	}

	if (idx >= board->num_ports) {
		dev_err(&pdev->dev, "Bad port index (%ld).", idx);
		return -EINVAL;
	}

	if (i >= count) {
		dev_err(&pdev->dev, "Invalid uart_mode.");
		return -EINVAL;
	}

	pci_read_config_dword(pdev, PERICOM_DRIVER_SEL, &driver_sel);
	driver_sel &= ~(PERICOM_DRIVER_SEL_PORT_MASK << DRIVER_SEL_PORT_SHIFT(idx, pdev->device));

	mode = &buf[i];
	if (strnstr(mode, "422", sizeof("422"))) {
		driver_sel |= (PERICOM_DRIVER_SEL_422 << DRIVER_SEL_PORT_SHIFT(idx, pdev->device));
	} else if (strnstr(mode, "485_2W", sizeof("485_2W"))) {
		driver_sel |= (PERICOM_DRIVER_SEL_485_2W << DRIVER_SEL_PORT_SHIFT(idx, pdev->device));
	} else if (strnstr(mode, "485_4W", sizeof("485_4W"))) {
		driver_sel |= (PERICOM_DRIVER_SEL_485_4W << DRIVER_SEL_PORT_SHIFT(idx, pdev->device));
	} else {
		driver_sel |= (PERICOM_DRIVER_SEL_232 << DRIVER_SEL_PORT_SHIFT(idx, pdev->device));
	}

	dev_dbg(&pdev->dev, "idx: %ld, DRIVER_SELECT: 0x%08x\n", idx, driver_sel);
	pci_write_config_dword(pdev, PERICOM_DRIVER_SEL, driver_sel);

	return count;
}

static ssize_t uart_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct pci_dev *pdev = container_of(dev, struct pci_dev, dev);
	struct serial_private *priv = pci_get_drvdata(pdev);
	const struct pciserial_board *board = priv->board;
	...
	pci_read_config_dword(pdev, PERICOM_DRIVER_SEL, &driver_sel);
	dev_dbg(&pdev->dev, "show:  DRIVER_SELECT: 0x%08x\n", driver_sel);
	for (i = 0; i < board->num_ports; i++) {
		count += snprintf(buf + count, PAGE_SIZE - count, "%d - ", i);

		tmp = driver_sel & (PERICOM_DRIVER_SEL_PORT_MASK << DRIVER_SEL_PORT_SHIFT(i, pdev->device));
		switch (tmp >> DRIVER_SEL_PORT_SHIFT(i, pdev->device)) {
			case PERICOM_DRIVER_SEL_422:
				count += snprintf(buf + count, PAGE_SIZE - count, "422\n");
				break;
			case PERICOM_DRIVER_SEL_485_2W:
				count += snprintf(buf + count, PAGE_SIZE - count, "485_2W\n");
				break;
			case PERICOM_DRIVER_SEL_485_4W:
				count += snprintf(buf + count, PAGE_SIZE - count, "485_4W\n");
				break;
			case PERICOM_DRIVER_SEL_232:
				count += snprintf(buf + count, PAGE_SIZE - count, "232\n");
							break;
			default:
				count += snprintf(buf + count, PAGE_SIZE - count, "Invalid!\n");
				break;
		}
	}
...
static struct attribute *pericom_attrs[] = {
	&dev_attr_uart_mode.attr,
	NULL,
};
static struct attribute_group pericom_attr_group = {
	.attrs = pericom_attrs,
};

int pci_pericom_init(struct pci_dev *dev)
{
	int ret;

	ret = sysfs_create_group(&dev->dev.kobj, &pericom_attr_group);
	dev_dbg(&dev->dev, "sysfs_create_group: %d\n", ret);

	return ret;
}

static int pericom_rs485_config(struct uart_port *port,
...
static const struct serial_rs485 pci_pericom_rs485_supported = {
	.flags = SER_RS485_ENABLED
};

static void pericom_throttle(struct uart_port *port)
{
	unsigned long flags;
	struct uart_8250_port *up = up_to_u8250p(port);
	...
	if (up->flow_mcr) {
		...
		if (up->flow_mcr & UART_MCR_RTS)
			mctrl |= TIOCM_RTS;
		if (up->flow_mcr & UART_MCR_DTR)
			mctrl |= TIOCM_DTR;
		port->mctrl &= ~mctrl;
		old = serial_in(up, UART_MCR);
		serial_out(up, UART_MCR, old & ~up->flow_mcr);
	}
	spin_unlock_irqrestore(&port->lock, flags);
}

static void pericom_unthrottle(struct uart_port *port)
{
	unsigned long flags;
	struct uart_8250_port *up = up_to_u8250p(port);
...
#if IS_ENABLED(CONFIG_GPIO_PERICOM) || IS_ENABLED(CONFIG_GPIO_PERICOM_MODULE)
/* All pericom uarts have 8 gpio */
static const struct property_entry pericom_gpio_props[] = {
	PROPERTY_ENTRY_U32("ngpios", 8),
	{}
};

static void *pericom_register_gpio(struct pci_dev *pcidev)
{
	struct platform_device *pdev;

	pdev = platform_device_alloc("gpio_pericom", PLATFORM_DEVID_AUTO);
	if (!pdev)
		return NULL;

	pdev->dev.parent = &pcidev->dev;

	if (device_create_managed_software_node(&pdev->dev, pericom_gpio_props, NULL) < 0 ||
...
static void pci_pericom_exit(struct pci_dev *pcidev)
{
	struct serial_private *priv = pci_get_drvdata(pcidev);
...
#endif // defined(CONFIG_GPIO_PERICOM) || defined(CONFIG_GPIO_PERICOM_MODULE)

static int pci_pericom_setup_mm(struct serial_private *priv,
		  const struct pciserial_board *board,
		  struct uart_8250_port *port, int idx, int quirk_idx)
{
	unsigned int bar, offset = board->first_offset, maxnr;
	int ret;
	...
	port->port.type = PORT_DIGI_PERICOM_PI7C9X795X;
	port->port.flags |= UPF_AUTO_CTS | UPF_FIXED_TYPE;

	port->port.throttle = pericom_throttle;
	port->port.unthrottle = pericom_unthrottle;

	port->port.rs485_config = pericom_rs485_config;
	port->port.rs485_supported = pci_pericom_rs485_supported;

	ret = setup_port(priv, port, bar, offset, board->reg_shift);
	if (ret != 0)
		goto out;

#if defined(CONFIG_GPIO_PERICOM) || defined(CONFIG_GPIO_PERICOM_MODULE)
	/* While setting up the first port, register the board gpio */
	if (idx == 0) {
		struct pci_dev *pcidev = priv->dev;
		port->port.private_data = pericom_register_gpio(pcidev);
...
/* Last port of the quad uart has an offset of 0xE00, so quirk_idx == 3 */
#define PERICOM_QUAD_UART_QUIRK_IDX 3
#define PERICOM_NO_QUIRK_PORT -1
static int pci_pericom_setup_four_at_five_twelve(struct serial_private *priv,
          const struct pciserial_board *board,
          struct uart_8250_port *port, int idx)
{
	return pci_pericom_setup_mm(priv, board, port, idx, PERICOM_QUAD_UART_QUIRK_IDX);
}

static int pci_pericom_setup_eight_at_five_twelve(struct serial_private *priv,
          const struct pciserial_board *board,
          struct uart_8250_port *port, int idx)
{
	return pci_pericom_setup_mm(priv, board, port, idx, PERICOM_NO_QUIRK_PORT);
}
#endif // CONFIG_SERIAL_8250_PERICOM_DIGI_EXTENSIONS

#ifndef CONFIG_SERIAL_8250_PERICOM_DIGI_EXTENSIONS
...
static struct pci_serial_quirk pci_serial_quirks[] = {
	...
	*/
...
#ifdef CONFIG_SERIAL_8250_PERICOM_DIGI_EXTENSIONS
	{
		...
		.device		= PCI_DEVICE_ID_PERICOM_PI7C9X7952,
		.subvendor	= PCI_ANY_ID,
		.subdevice	= PCI_ANY_ID,
		.init       = pci_pericom_init,
		.setup		= pci_pericom_setup_four_at_five_twelve,
#if defined(CONFIG_GPIO_PERICOM) || defined(CONFIG_GPIO_PERICOM_MODULE)
		.exit		= pci_pericom_exit,
#endif
	},
#endif
	{
	...
	{
		.vendor		= PCI_VENDOR_ID_PERICOM,
		.device		= PCI_DEVICE_ID_PERICOM_PI7C9X7958,
		.subvendor	= PCI_ANY_ID,
		.subdevice	= PCI_ANY_ID,
		.init       = pci_pericom_init,
		.setup		= pci_pericom_setup_eight_at_five_twelve,
#if defined(CONFIG_GPIO_PERICOM) || defined(CONFIG_GPIO_PERICOM_MODULE)
		.exit		= pci_pericom_exit,
#endif
	},
...
enum pci_board_num_t {
	...
	pbn_moxa_8,
#ifdef CONFIG_SERIAL_8250_PERICOM_DIGI_EXTENSIONS
	pbn_pericom_PI7C9X7952_mapped,
	pbn_pericom_PI7C9X7954_mapped,
	pbn_pericom_PI7C9X7958_mapped,
#endif
};
...
/*	{	PCI_VENDOR_ID_KORENIX, PCI_DEVICE_ID_KORENIX_JETCARDF0,
	...
	{   PCI_VENDOR_ID_PERICOM, PCI_DEVICE_ID_PERICOM_PI7C9X7952,
		PCI_ANY_ID, PCI_ANY_ID,
		0,
		0, pbn_pericom_PI7C9X7952_mapped },
	{   PCI_VENDOR_ID_PERICOM, PCI_DEVICE_ID_PERICOM_PI7C9X7954,
		PCI_ANY_ID, PCI_ANY_ID,
		0,
		0, pbn_pericom_PI7C9X7954_mapped },
	{   PCI_VENDOR_ID_PERICOM, PCI_DEVICE_ID_PERICOM_PI7C9X7958,
		PCI_ANY_ID, PCI_ANY_ID,
		0,
		0, pbn_pericom_PI7C9X7958_mapped },
#endif
	/*
