...
#define ST_LIS2HH12_DEV_ATTR_TEMP_RAW() \
		IIO_DEV_ATTR_TEMP_RAW( \
			lis2hh12_sysfs_get_raw_temperature)

#define IIO_DEV_ATTR_TEMP_ABS(_show) \
		IIO_DEVICE_ATTR(in_temp_abs, S_IRUGO, _show, NULL, 0)

#define ST_LIS2HH12_DEV_ATTR_TEMP_ABS() \
		IIO_DEV_ATTR_TEMP_ABS( \
			lis2hh12_sysfs_get_abs_temperature)
...
static u32 temp_offset = 25000;
module_param(temp_offset, int, 0644);
MODULE_PARM_DESC(temp_offset, "Temperature sensor offset in millicelsius");
...
static int get_temp_raw_value(struct device *dev, int16_t *raw_val)
{
	...
	/*
	 * Give the I2C device some time to sample the data
	 * before starting to read them
	 */
	msleep(40);

	err = lis2hh12_read_register(sdata->cdata, LIS2HH12_TEMP_L_ADDR, 2, outdata);
	if (err < 0)
		goto out;

	*raw_val = (outdata[1] << 8) | outdata[0];
					...
					char *buf)
{
	...
	/* Use only the upper 11Bits of the data */
	raw_value = (raw_value >> 5);

	/* Calculate absolute temperature */
	temp_millicelsius = (raw_value * LIS2HH12_TEMP_FRACTION_MULTIPLIER) + temp_offset;
...
static ST_LIS2HH12_DEV_ATTR_TEMP_RAW();
static ST_LIS2HH12_DEV_ATTR_TEMP_ABS();
...
static struct attribute *lis2hh12_accel_attributes[] = {
	...
	&iio_dev_attr_in_temp_abs.dev_attr.attr,
	NULL,
};
