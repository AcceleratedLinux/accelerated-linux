...
qca8k_setup_mdio_bus(struct qca8k_priv *priv)
{
	...
	for_each_available_child_of_node(ports, port) {
		...
		/* If port 6 is in SGMII mode then treat it is an SFP */
		if ((reg == 6) && (mode == PHY_INTERFACE_MODE_SGMII))
			continue;
...
static int
qca8k_setup_led_ctrl_of(struct qca8k_priv *priv)
{
	struct device_node *node = priv->dev->of_node;
	u32 val;

	/* LED control settings are optional */
	if (!of_property_read_u32(node, "qca,led-ctrl0", &val))
		qca8k_write(priv, QCA8K_LED_CTRL0_REG, val);
	if (!of_property_read_u32(node, "qca,led-ctrl1", &val))
		qca8k_write(priv, QCA8K_LED_CTRL1_REG, val);
	if (!of_property_read_u32(node, "qca,led-ctrl2", &val))
		qca8k_write(priv, QCA8K_LED_CTRL2_REG, val);
			  ...
			  bool tx_pause, bool rx_pause)
{
	...
	if ((port == 6) && dsa_is_user_port(dp->ds, port)) {
		/* Port is in SGMII mode with external phy (ie SFP) */
...
qca8k_setup(struct dsa_switch *ds)
{
	...
	ret = qca8k_setup_led_ctrl_of(priv);
	if (ret)
		return ret;
...
static int qca8k_8xxx_port_enable(struct dsa_switch *ds, int port,
				  struct phy_device *phy)
{
	struct qca8k_priv *priv = ds->priv;

	if (port != 6)
		return qca8k_port_enable(ds, port, phy);
...
static const struct dsa_switch_ops qca8k_switch_ops = {
	...
	.port_enable		= qca8k_8xxx_port_enable,
