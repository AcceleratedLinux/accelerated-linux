				  ...
				  struct ethtool_pauseparam *epause)
{
	...
	ethtool_convert_link_mode_to_legacy_u32(&supported, priv->phydev->supported);
	ethtool_convert_link_mode_to_legacy_u32(&advertising, priv->phydev->advertising);
	...
	ethtool_convert_legacy_u32_to_link_mode(priv->phydev->supported, supported);
	ethtool_convert_legacy_u32_to_link_mode(priv->phydev->advertising, advertising);
...
phy_init:
	...
	if ((priv->einfo->mii_config == PHY_INTERFACE_MODE_RGMII) ||
		...
		regmap_read(pfe->scfg, RGMIIPCR, &rgmii_pcr);
		rgmii_pcr  &= ~(0xFF);
		rgmii_pcr |= SCFG_RGMIIPCR_EN_AUTO;
		regmap_write(pfe->scfg, RGMIIPCR, rgmii_pcr);
	}
...
/*
 * We are using this driver stand-alone, and not in NXP's vendor source.
 * So we don't have the fsl support functions available without pulling
 * a lot of mostly un-necessary code. So for now fake out the one extra
 * function we do need. I think it is safe to assume that we will always
 * be running on post 1.0 revision silicon.
 */
static int fsl_guts_get_svr(void)
{
	return LS1012A_REV_1_0 + 1;
}
