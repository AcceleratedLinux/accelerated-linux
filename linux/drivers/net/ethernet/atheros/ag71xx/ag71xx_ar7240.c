...
#ifdef CONFIG_LEDMAN
static unsigned int linkledmap[] = {
	LEDMAN_LAN4_LINK, LEDMAN_LAN3_LINK, LEDMAN_LAN2_LINK, LEDMAN_LAN1_LINK,
};
#endif

static inline void setlinkled(int port, int link)
{
#ifdef CONFIG_LEDMAN
	ledman_cmd(link ? LEDMAN_CMD_ON : LEDMAN_CMD_OFF, linkledmap[port]);
#endif
}
...
static void link_function(struct work_struct *work) {
	...
	for (i = 0; i < AR7240_NUM_PHYS; i++) {
		...
		setlinkled(i, link & BMSR_LSTATUS);
	}
...
int ag71xx_ar7240_mii_ioctl(struct ag71xx *ag, struct ifreq *ifr, int cmd)
{
	...
	switch (cmd) {
	case SIOCGMIIPHY:
		data->phy_id = AR7240_NUM_PHYS; /* use a fake phyid as special case */
		break;

	case SIOCGMIIREG:
		/* if you specify a real phy,  use it */
		if (data->phy_id >= 0 && data->phy_id < AR7240_NUM_PHYS) {
			data->val_out = ar7240sw_phy_read(ag->mii_bus, data->phy_id,
						data->reg_num);
			return 0;
		}
			
		if (data->phy_id == AR7240_NUM_PHYS) {
			/* use the first port with link,  looks realish then */
			for (phy = 0; phy < AR7240_NUM_PHYS; phy++) {
				/* read status */
				data->val_out = ar7240sw_phy_read(ag->mii_bus,phy,1);
				if (data->val_out & 0x4)
					break;
			}
			if (phy >= AR7240_NUM_PHYS)
				phy = 0;
			data->val_out = ar7240sw_phy_read(ag->mii_bus, phy, data->reg_num);
			return 0;
		}
	...
	case SIOCSMIIREG:
		/* if you specify a real phy, we write to it */
		if (data->phy_id >= 0 && data->phy_id < AR7240_NUM_PHYS)
			return ar7240sw_phy_write(ag->mii_bus, data->phy_id, data->reg_num,
						data->val_in);
		/* writes to the fake phy do nothing */
		if (data->phy_id == AR7240_NUM_PHYS)
			return 0;
		return -EINVAL;
		...
		struct ethtool_cmd *cmd)
{
	...
	for (phy = 0; phy < AR7240_NUM_PHYS; phy++) {
		/* read status */
		status = ar7240sw_phy_read(ag->mii_bus,phy,1);
		if (status & 0x4)
			break;
	}
	if (phy >= AR7240_NUM_PHYS)
