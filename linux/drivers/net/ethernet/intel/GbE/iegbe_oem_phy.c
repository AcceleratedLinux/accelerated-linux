...
iegbe_oem_setup_link(struct iegbe_hw *hw)
{
    ...
    switch (hw->phy_id) {
        ...
        case M88E1141_E_PHY_ID:
            ...
            break; 
	case M88E6161_E_PHY_ID:
	    /* Switch is _always_ connected at back end */
            hw->icp_xxxx_is_link_up = 1;
...
iegbe_oem_detect_phy(struct iegbe_hw *hw)
{
    ...
    /*
     * The attached Marvel M88E6161 switches are mapped indirectly through
     * the PHY address space. So we have to hard code the IDs and detection
     * here.
     */
    if (iegbe_oem_get_dev_number(hw) != 0) {
	hw->phy_id = M88E6161_E_PHY_ID;
	hw->phy_revision = 0;
	return E1000_SUCCESS;
    }
...
#if defined(CONFIG_UTM2000) || defined(CONFIG_UTM3000)
    /* SnapGear Tolapi board */
    switch(device_number) {
        case ICP_XXXX_MAC_1:    hw->phy_addr = 64; break;
        case ICP_XXXX_MAC_2:    hw->phy_addr = 128; break;
        default:                hw->phy_addr = 1; break;
    }
#else
	switch(device_number)
    {
...
uint32_t 
iegbe_oem_get_dev_number(struct iegbe_hw *hw)
{
#ifdef EXTERNAL_MDIO
    /* 
     * for ICP_XXXX family of devices, the three network interfaces are 
    ...
    if (device_number == ICP_XXXX_MAC_1)
	return 1;
...
iegbe_oem_phy_speed_downgraded(struct iegbe_hw *hw, uint16_t *isDowngraded)
{
   ...
   DEBUGFUNC1("%s",__func__);
    ...
    switch (hw->phy_id) {
        ...
        break; 
	case M88E6161_E_PHY_ID:
	    /* Attached switch, no down grading master port */
	break;
        default:
            DEBUGOUT("Invalid PHY ID\n");
            return 1;
...
iegbe_oem_phy_is_speed_1000(struct iegbe_hw *hw, int *is1000)
{
    ...
    switch (hw->phy_id) {
	 ...
	 break;
         ...
         case M88E6161_E_PHY_ID:
	    /* Always 1Gb for back end of switch */
            *is1000 = 1;
	 break;
...
int32_t
iegbe_oem_read_phy_phy_reg_ex(struct iegbe_hw *hw,
			      uint32_t phy_addr,
			      uint32_t reg_addr,
			      uint16_t *phy_data)
{
     ...
     */
        ...
        ret_val = gcu_read_eth_phy(phy_addr, reg_addr, phy_data);
        if(ret_val) {
            DEBUGOUT("Error reading GCU");
            return ret_val;
        }
...
int32_t
iegbe_oem_write_phy_phy_reg_ex(struct iegbe_hw *hw,
			       uint32_t phy_addr,
			       uint32_t reg_addr,
			       uint16_t phy_data)
{
   ...
   DEBUGFUNC1("%s",__func__);
     ...
     *       gcu module
     */
        ret_val = gcu_write_eth_phy(phy_addr, reg_addr, phy_data);
        if(ret_val) {
            DEBUGOUT("Error writing to GCU");
            return ret_val;
        }
                           ...
                           uint16_t *max_length)
{
   ...
   DEBUGFUNC1("%s",__func__);
    ...
    switch (hw->phy_id) {
        ...
        break;
	case M88E6161_E_PHY_ID:
	    /* Means nothing on fixed local switch */
	case VSC8601_E_PHY_ID:
	    *min_length = 0;
	    *max_length = iegbe_igp_cable_length_170;
	break;
...
iegbe_oem_phy_is_link_up(struct iegbe_hw *hw, int *isUp)
{
    ...
    switch (hw->phy_id) {
        ...
        break; 
        case M88E6161_E_PHY_ID:
	    /* Local switch, we always have link to it */
            *isUp = 1;
            return E1000_SUCCESS;
        break;
        case VSC8601_E_PHY_ID:
