...
static int vsc85xx_dt_led_inverted_get(struct phy_device *phydev, char *led)
{
	struct device *dev = &phydev->mdio.dev;
	struct device_node *of_node = dev->of_node;
	...
	return (of_get_property(of_node, led, NULL) != NULL);
}
				    ...
				    u32 *default_mode)
{
	...
	for (i = 0; i < priv->nleds; i++) {
		...
		ret = sprintf(led_dt_prop, "vsc8531,led-%d-inverted", i);
		if (ret < 0)
			return ret;

		ret = vsc85xx_dt_led_inverted_get(phydev, led_dt_prop);
		if (ret < 0)
			return ret;
		if (ret)
			priv->leds_inverted |= BIT(INVERT_LED_POL_POS + i);
	}
...
static int vsc8584_config_init(struct phy_device *phydev)
{
	...
	if (vsc8531->leds_inverted)
		phy_modify_paged(
			phydev, MSCC_PHY_PAGE_EXTENDED_2, MSCC_PHY_EEE_CNTL,
			GENMASK(INVERT_LED_POL_POS,
				INVERT_LED_POL_POS + vsc8531->nleds - 1),
			vsc8531->leds_inverted);
