...
static int dp83867_verify_rgmii_cfg(struct phy_device *phydev)
{
	...
	/*
	 * We want to be able to support devicetree configuration without
	 * a specific DP83867 configuration entry. So even if we are using
	 * rgmii-id mode (that is "internel delay") allow no ID parameters
	 * to be listed. In this case keep using the existing delay already
	 * set into the PFE dp83867 registers.
	 */
#if 0
	struct dp83867_private *dp83867 = phydev->priv;
...
static int dp83867_config_init(struct phy_device *phydev)
{
	...
	if (phy_interface_is_rgmii(phydev)) {
		...
		if (dp83867->rx_id_delay != DP83867_RGMII_RX_CLK_DELAY_INV) {
			delay &= ~DP83867_RGMII_RX_CLK_DELAY_MAX;
