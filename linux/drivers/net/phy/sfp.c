...
static void sfp_init_power(struct sfp *sfp)
{
	struct gpio_desc *pwr_gpio;

	pwr_gpio = devm_gpiod_get_optional(sfp->dev, "power", GPIOD_OUT_LOW);
	if (IS_ERR(pwr_gpio)) {
		dev_warn(sfp->dev, "acquiring SFP power GPIO failed");
	} else {
		/* Make sure we had at least some power off time */
		usleep_range(10000, 25000);
		gpiod_set_value_cansleep(pwr_gpio, 1);
	}
}
