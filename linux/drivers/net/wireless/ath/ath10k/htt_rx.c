				       ...
				       struct sk_buff *skb)
{
	...
	for (i = 0; i < num_airtime_records; i++) {
		...
		/*
		 * Peer->sta can be NULL when a device is connecting to an AP. Don't
		 * collect durations from a device until the sta is non-null.
		*/
		if (peer->sta)
...
static void ath10k_fetch_10_2_tx_stats(struct ath10k *ar, u8 *data)
{
	...
	if (!peer || !peer->sta) {
		/*
		Commented out this line as we will always get this error if
		we have a defined client which can't connect. The interface
		is up, but the station/peer will be null until we connect.
		We don't need to see this error/warning.
