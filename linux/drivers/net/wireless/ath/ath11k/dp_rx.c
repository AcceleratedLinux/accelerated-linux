...
//SparkLAN for Monitor Mode signal
static void ath11k_dp_rx_h_signal(struct ath11k *ar, struct ieee80211_rx_status *rxs)
{
    struct ath11k_pdev_dp *dp = &ar->dp;
    struct ath11k_mon_data *pmon = (struct ath11k_mon_data *)&dp->mon_data;
    ...
    for (i = 0; i < IEEE80211_MAX_CHAINS ; i++) {
        rxs->chains &= ~BIT(i);
        rssi = ppdu_info->rssi_chain_pri20[i];
        if (rssi != ATH11K_DEFAULT_NOISE_FLOOR &&
            rssi != ATH11K_INVALID_RSSI &&
            rssi != ATH11K_INVALID_RSSI_2 &&
            rssi != 0) {
            rxs->chain_signal[i] = rssi;
            rxs->chains |= BIT(i);
        }
    }

    rssi = ppdu_info->rssi_comb + ATH11K_DEFAULT_NOISE_FLOOR;
    rxs->signal = rssi;
