...
static void ath11k_pci_init_qmi_ce_config(struct ath11k_base *ab)
{
	...
	case ATH11K_HW_QCN9074_HW10:
		/*
		 * All SoC-s are standing up with the same instance ID, which
		 * makes impossible for the host to decide which one it is
		 * talking to. Change it based on the PCI bus. MHI driver
		 * already changed this in the device itself
		 */
		instance =
			((pci_domain_nr(bus) & 0xF) << 4) | (bus->number & 0xF);
		instance &= QRTR_INSTANCE_MASK;

		ab->qmi.service_ins_id += instance;

		break;
	default:
		/* No need to set node ID, or not supported yet */
		break;
	}
