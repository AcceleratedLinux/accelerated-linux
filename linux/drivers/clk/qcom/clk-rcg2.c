			...
			unsigned long rate, enum freq_policy policy)
{
	...
	for (freq = f->freq; freq == f->freq; f++) {
		...
		parent_temp = clk_hw_get_parent_by_index(hw, index);
		if (!parent_temp) {
			pr_info("[FIXME]: parent index %d of %s (parent_num: %x) is NULL\n", index, clk_hw_get_name(hw), clk_hw_get_num_parents(hw));
			continue;
		}

		rate = clk_hw_get_rate(parent_temp);
