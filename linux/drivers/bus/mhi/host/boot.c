...
skip_req_fw:
	...
	/*
	 * Write QRTR node ID into the BHI_ERRDBG2 register on QCN9000 chips.
	 * It has to be unique so each node can be addresses, and make the IPC
	 * functioning
	 */
	if (pci_dev->vendor == QCN9000_VENDOR_ID &&
		...
		mhi_write_reg_field(mhi_cntrl, mhi_cntrl->bhi, BHI_ERRDBG2,
				    QRTR_INSTANCE_MASK, instance);
	}
