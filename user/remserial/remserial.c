...
extern int connect_to(struct sockaddr_in *addr);
extern void usage(char *progname);
extern void link_slave(int fd);
...
int main(int argc, char *argv[])
{
	...
	while ( (c=getopt(argc,argv,"dl:m:p:r:s:wx:")) != EOF )
		...
		case 'm':
            ...
            if ((maxConnects < 1) || (maxConnects > 5))
            {
                syslog(LOG_DEBUG, "Invallid value for max. number of simultaneous connections");
                exit(1);
            }
			break;
		case 'p':
    ...
    if (remotefd == NULL)
    {
        syslog(LOG_DEBUG, "Allocation of remote file descriptors failed");
        exit(1);
    }
	...
	*/
	...
	if ( devfd == -1 ) {
		syslog(LOG_DEBUG, "Open of %s failed: %m",sdevname);
		exit(1);
	}
	...
	if ( machinename ) {
		...
		if ( !remotehost ) {
			syslog(LOG_DEBUG, "Couldn't determine address of %s",
				machinename );
			exit(1);
		}
		...
		if ( sockfd == -1 ) {
			syslog(LOG_DEBUG, "Can't open socket: %m");
			exit(1);
		}
		...
		if( bind( sockfd, (struct sockaddr*)(&addr),
			...
			syslog(LOG_DEBUG, "Couldn't bind port %d, aborting: %m",port );
			exit(1);
		}
		if ( debug>1 )
		...
		if ( result == -1 ) {
			syslog(LOG_DEBUG, "Socket listen failed: %m");
			exit(1);
		}
		...
		if ( FD_ISSET(devfd,&fdsreaduse) ) {
			...
			if ( devbytes <= 0 ) {
				...
				while (1) {
					...
					if ( devfd != -1 )
						break;
					syslog(LOG_DEBUG, "Open of %s failed: %m",
						sdevname);
					if ( errno != EIO )
						exit(1);
					sleep(1);
...
void sighandler(int sig)
{
	...
	if (linkname)
		unlink(linkname);
	syslog(LOG_DEBUG,"Terminating on signal %d",sig);
	exit(0);
}
...
void link_slave(int fd)
{
	...
	if (status == -1) {
		syslog(LOG_DEBUG, "Cannot create link for pseudo-tty: %m");
		exit(1);
	}
}
