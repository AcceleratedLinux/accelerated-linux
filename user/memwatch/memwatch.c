 ...
 ***********************************************************************/
static void print_hex_formated( uint32_t val, access_t access )
{
	switch( access ) {
		case BYTE:	printf( "%02x ", val );	break;
		case WORD:	printf( "%04x ", val );	break;
		case DWORD:	printf( "%08x ", val );	break;
	}
}
...
{
	int read_write = 1, len = -1, pending, chunk, verbose = 0, j;
	...
	static const char *short_options = "rxbhwsl:a:d:f:v:";
	...
	pending = len;
	while( pending ) {
		chunk = (pending > MAP_SIZE) ? MAP_SIZE : pending;
		...
		for( j = 0; j < chunk; j += access ) {
			if( read_write ) {
				switch( access ) {
					case BYTE:
						rd_val = *((uint8_t*) virt_addr);
						break;
					case WORD:
						rd_val = *((uint16_t*) virt_addr);
						if ( swap )
							rd_val = __bswap_16( rd_val );
						break;
					case DWORD:
						rd_val = *((uint32_t*) virt_addr);
						if ( swap )
							rd_val = __bswap_32( rd_val );
						break;
				}
				if( filename != NULL ) {
					if( write( fd_file, (uint8_t*)&rd_val, access ) != access ) {
				...
				switch( access ) {
					case BYTE:	*((uint8_t*) virt_addr) = wr_val;	break;
					case WORD:	*((uint16_t*) virt_addr) = swap ? __bswap_16( wr_val ) : wr_val;	break;
					case DWORD:	*((uint32_t*) virt_addr) = swap ? __bswap_32( wr_val ) : wr_val;	break;
				}
			}
			virt_addr += access;
		}
