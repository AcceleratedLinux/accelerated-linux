...
static void randiv(unsigned char *iv, int size)
{
	if (!RAND_bytes(iv, size))
		printf("WARNING: failed to get random bytes (%d wanted)\n", size);
}
...
int main(int argc, char *argv[]) {
	...
	pname = argv[0];
	while ((opt = getopt(argc, argv, "FnveEtf:k:")) > 0) {
		switch (opt) {
	...
	if (encp) {
		...
		if (verbose)
			printf("%s requires %d bytes padding\n",
					fname, hdr.padsize);
		if (encp == 2) {
			/* Add extra space for storing the IV */
			dsize += AES_BLOCK_SIZE;
			if (verbose)
				printf("%s requires %d bytes initial vector\n",
					fname, AES_BLOCK_SIZE);
		}
	}
	fmem = malloc(dsize);
	if (fmem == NULL)
	...
	if (encp == 2) {
		...
		size_t len = dsize - START_POSN - AES_BLOCK_SIZE;
		AES_KEY key;
		hdr.flags |= FLAG_CBCENCRYPTED;
		getkey(hdr.aeskey);
		AES_set_encrypt_key(hdr.aeskey, AESKEYSIZE * 8, &key);
		randiv(iv, AES_BLOCK_SIZE);
		memcpy(fmem+START_POSN+len, iv, AES_BLOCK_SIZE);
		AES_cbc_encrypt(fmem+START_POSN, fmem+START_POSN, len, &key, iv, 1);
	}
