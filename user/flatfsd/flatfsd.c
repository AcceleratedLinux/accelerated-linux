...
#ifdef CONFIG_USER_FLATFSD_MCA_UBOOTENV_FACTORYRESET
/*
 * For complete factory resets (.init2):
 *  - Factory reset the MCA using mca_tool
 *  - Signal U-Boot to factory reset its environment next reboot
 */
static void factoryreset_mca_and_ubootenv() {
	int ret;

	/* Only for 'complete' factory resets (exit if .init2 is not there) */
	if (access(SRCDIR "/.init2", F_OK) != 0)
		return;

	printf("Resetting MCA and U-Boot environment...\n");

	ret = system("mca_config_tool --factory_reset");
	if (ret)
		syslog(LOG_ERR, "Failed to perform MCA factory reset (%d)", ret);

	ret = system("fw_setenv factory_reset 1");
	if (ret)
		syslog(LOG_ERR, "Failed to flag U-Boot environment factory reset (%d)", ret);
}
#endif
