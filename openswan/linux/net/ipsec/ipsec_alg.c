...
int
ipsec_xform_show(struct seq_file *seq, void *offset)
{
	int i;
	...
	for (i =0, head = ipsec_alg_hash_table; i < IPSEC_ALG_HASHSZ; i++, head++) {
		...
		for (p = head->next; p != head; p = p->next) {
			...
			case IPSEC_ALG_TYPE_AUTH:
				...
				seq_printf(seq, "KEYLEN=%d CTXSIZE=%d AUTHLEN=%d ",
			...
			case IPSEC_ALG_TYPE_ENCRYPT:
				...
				seq_printf(seq, "KEYLEN=%d CTXSIZE=%d ",
