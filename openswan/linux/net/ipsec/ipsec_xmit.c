...
ipsec_xmit_ipip(struct ipsec_xmit_state *ixs)
{
...
#endif /* CONFIG_KLIPS_IPV6 */
	{
		...
		if (error != IPSEC_XMIT_OK)
			return error;
		KLIPS_IP_SELECT_IDENT(dev_net(ixs->dev), osw_ip4_hdr(ixs), ixs->skb);
		skb_set_transport_header(ixs->skb, ipsec_skb_offset(ixs->skb, ixs->iph));
	}
...
#define MY_NF_HOOK(pf,hook,skb,in,out,okfn)	NF_HOOK(pf,hook,dev_net((skb)->dev),NULL,skb,in,out,okfn)
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(4,1,0)
static inline int ipsec_xmit_send2(struct sock *sk, struct sk_buff *skb)
{
	return dst_output_sk(sk, skb);
}
#define MY_NF_HOOK(a,b,c,d,e,f)	NF_HOOK(a,b,NULL,c,d,e,f)
#else
...
#endif
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,0)
	return ipsec_xmit_send2(dev_net(skb->dev), NULL, skb);
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(4,1,0)
	return ipsec_xmit_send2(NULL, skb);
#else
...
ipsec_xmit_send(struct ipsec_xmit_state *ixs)
{
	...
	KLIPS_IP_PRINT(debug_tunnel & DB_TN_XMIT, ip_hdr(ixs->skb));
	{
		...
		if (ixs->mast_mode)
			...
			err = MY_NF_HOOK(PF_INET6, OSW_NF_INET_LOCAL_OUT, ixs->skb, NULL,
					ixs->route ? ipsec_route_dst(ixs->route).dev : skb_dst(ixs->skb)->dev,
					ipsec_xmit_send2);
		else
			err = MY_NF_HOOK(PF_INET, OSW_NF_INET_LOCAL_OUT, ixs->skb, NULL,
					ixs->route ? ipsec_route_dst(ixs->route).dev : skb_dst(ixs->skb)->dev,
					ipsec_xmit_send2);
