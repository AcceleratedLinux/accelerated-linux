#
# Common makefile for all Digi/IX30* variants
#

COMMON_DIR = $(ROOTDIR)/vendors/Digi/IX30
...
romfs.common: romfs_dev romfs.dirs romfs.default romfs.version romfs.cryptokey
	...
	$(ROMFSINST) -d -p 555 $(COMMON_DIR)/led /etc/action.d/led
	$(ROMFSINST) $(COMMON_DIR)/fw_env.config /etc/fw_env.config
	$(ROMFSINST) -s /var/tmp/log /dev/log
	$(ROMFSINST) -s /var/run /run
	$(ROMFSINST) -s /var/run/syslog.conf -e CONFIG_USER_SYSKLOGD /etc/syslog.conf
	$(ROMFSINST) -d $(COMMON_DIR)/console /etc/inittab.d/console
	$(ROMFSINST) -d -p 555 $(COMMON_DIR)/serial-mode /bin/serial-mode
	$(ROMFSINST) -p 555 $(COMMON_DIR)/rc /etc/rc
